...
public interface InventarioSedeRepository extends JpaRepository<Inventariosede,Integer> {
    ...
    @Query(value="SELECT * FROM inventariosede invs where invs.sede_idsede = (select idsede from sede where nombre = ?1);",nativeQuery=true)

    List<Inventariosede> obtenerInvDeMiSede(String sesionSede);
}
